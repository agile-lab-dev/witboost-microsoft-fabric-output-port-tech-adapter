class SQLSchemaMapper:
    ...
    def map_data_type(column):
        ...
        if column.dataType == "TEXT":
            return f"varchar({column.dataLength})" if column.dataLength else "varchar(max)"
        elif column.dataType == "INT":
            return "int"
        elif column.dataType == "DECIMAL":
            return f"decimal({column.precision},{column.scale})" if column.precision and column.scale else "decimal"
    ...
    def generate_sql_schema(schema, nullable=True):
        ...
        for col in schema:
            ...
            if sql_type.startswith("varchar"):
                sql_columns.append(f"\t[{col.name}] [varchar]({col.dataLength}) {nullability}")
            elif "decimal" in sql_type:
                sql_columns.append(f"\t[{col.name}] [decimal]({col.precision},{col.scale}) {nullability}")
            else:
